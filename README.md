...
*  **Watermark Overlay**: Automatically applies a transparent image overlay (watermark) to video files.
*    **AI Subtitles**: Extracts audio from video and generates `.srt` subtitle files using the **Groq API** (Whisper model).
*    YouTube Prospector**: Scrapes YouTube for videos based on specific niches, view counts, duration, and upload date filters to find viral leads.
